...
def process_one_image_pair(image_name,clean_folder, noise_folder,clean_t,noise_t):
    ...
    denoised_image = filtering(cur_noise_image, weight=1)
...
def main():
    ...
    parallel_worker = Pool(processes=200)
    for one_noise_type in noise_path:
        res[one_noise_type] = {}
        for one_level in noise_level:
            ...
            for one_cate in cate_list:
                ...
                # batch_res.append(process_one_image_pair(img, clean_folder=os.path.join(clean_path, one_cate),
                #                       noise_folder=os.path.join(one_noise_type, str(one_level), one_cate),
                #                       clean_t=clean_t, noise_t=noise_t))
